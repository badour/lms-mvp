...
[Area("Student")]
[Authorize(Roles = "Student,SuperAdministrator,SchoolAdministrator")]
public class DashboardController : Controller
{
    ...
    public async Task<IActionResult> Index(CancellationToken cancellationToken)
    {
        ...
        if (studentId == 0)
        {
            studentId = await _db.Students.AsNoTracking().Where(x => !x.IsDeleted).Select(x => x.Id).FirstOrDefaultAsync(cancellationToken);
        }

        var model = studentId == 0 ? null : await _dashboardService.GetStudentDashboardAsync(studentId, cancellationToken);
        return View(model);
    }
}
